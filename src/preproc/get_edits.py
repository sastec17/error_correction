...
def get_edits(sql_clauses, gold_sql_clauses, sql_label, gold_sql_label, schema):
    """
        ...
        sql_label:
        gold_sql_label:
        schema: Schema(schema, table) - All essential database information
    
    """
    # Just store actions in a list
    actions = []
    for k in SQL_DICT_KEYS:
        # If predicted label / clause is incorrect compared to gold_sql_label - Need to do something
        if sql_label[k] != gold_sql_label[k]:
            if k == 'limit':
                # If supposed to have stuff that prediction doesn't have - need to insert gold clause
                if sql_label[k] is None or sql_label[k] == 0:
                    actions.append(['insert', k, gold_sql_clauses[k]])
                # If we have stuff we don't want - Need to delete incorrect prediction
                elif gold_sql_label[k] is None or gold_sql_label[k] == 0:
                    actions.append(['delete', k])
                else:
                    # Otherwise, must replace with correct 
